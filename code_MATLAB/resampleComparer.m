...
data = "load"; % "new" "load" "empirical" "empiricalNew"
disp("Data: " + data)
...
if data == "new"
    ...
    param.length = [500 1000 2000 4000 8000]; % Length of track (in 'noise steps'), first is default
    ...
    param.angleMu = [30 40 50 60 70]; % Turn angles
    param.angleSD = [1 5 10 15 25]; % Std for turn angles: Must !=0 (otherwise Inf results)
    ...
    param.smoothW = [0 10 20 30]; % Smoothing window of LOWESS [noise steps], (def: 0:20)
    ...
    params(params.noiseAngle >= params.angleMu & params.angleMu~=0,:) = []; % noise > turn isn't really noise?
    params(params.angleMu == 0 & params.noiseAngle == 0 & params.noiseXY == 0,:) = []; % Removing straight line
    params(params.smoothW > 0 & (params.noiseAngle > 0 | params.angleSD > param.angleSD(1)),:) = []; % Either smooth OR angle noise/SD
    params(params.angleSD > param.angleSD(1) & (params.angleMu == param.angleMu(2) | params.angleMu == param.angleMu(4)),:) = []; % Only need 3 values to show tendency...
    params(params.noiseAngle > param.noiseAngle(1) & (params.angleMu == param.angleMu(2) | params.angleMu == param.angleMu(4)),:) = []; % ...in noise & SD analysis
    params(params.noiseAngle > param.noiseAngle(1) & params.angleSD > param.angleSD(1),:) = []; % Not interested in noise x SD interactions
    params(params.length > param.length(2) & (params.angleMu ~= param.angleMu(3) | params.angleSD > param.angleSD(1) |...
           params.noiseAngle > param.noiseAngle(1) | params.smoothW > param.smoothW(1)),:) = []; % Only keeping 1 param set per track length
...
elseif data == "load"
    dataFolderName = uigetdir("resample/");
...
xValName = 'angleMu'; % angleMu (fig 4), angleSD, noiseAngle (fig S 'noise'), or length (fig S 'length')
yValName = 'angleMu';
