...
%        c) n x 1 matrix. Must be supplied with 'group' vector
%
% varargin:
% group:     Grouping vector. Vector of IDs (could be anything) grouping the data into violins
...
x = [];
%_____________________
if isempty(find(strcmp(varargin,'group'),1))==0 % A grouping vector has been supplied
    grpVec = varargin{find(strcmp(varargin,'group'))+1};
    if ~isvector(grpVec) || numel(grpVec) ~= numel(Y)
        error('Grouping vector must be a vector of same length as the data vector')
    end
    [~,~,gId] = unique(grpVec);
    Y = splitapply(@(x){x}, Y, gId)';
end
...
bw = nan(1,size(Y,2));
for i=1:size(Y,2)
    if ~all(isnan(Y{i})) % NaNs will not be plotted (otherwise error)
        if isempty(b)==0
...
%% Plot the violins
for i = 1:size(Y,2)
    if ~isnan(Y{i}) % NaNs will not be plotted (otherwise error)
        if isempty(lc) == 1
            if setX == 0
